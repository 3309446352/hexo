  ...
  <div class="flex justify-event flex-wrap pt-2">
    <div id="article-container">
      <div class="bangumi-tabs">
        <a class="bangumi-tab" id="bangumi-tab1" @click.stop="bangumiTab('wantWatch')"
          :class="{ 'bangumi-active': currentTab === 'wantWatch' }"> 想看{{ state.wantWatch.length }}</a>
        <a class="bangumi-tab" id="bangumi-tab2" @click.stop="bangumiTab('watching')"
          :class="{ 'bangumi-active': currentTab === 'watching' }">在看{{ state.watching.length }}</a>
        <a class="bangumi-tab" id="bangumi-tab3" @click.stop="bangumiTab('watched')"
          :class="{ 'bangumi-active': currentTab === 'watched' }">已看{{ state.watched.length }}</a>
      </div>
      <div>
        <div id="bangumi-item" class="bangumi-show" :key="currentTab">
          <div class="bangumi-item" v-for="(item, index) in watchTab" :key="item.subject_id || index">
...
export default defineComponent({
  ...
  setup() {
    ...
    // 使用 reactive 保持响应式
    const currentTab = ref<'watched' | 'watching' | 'wantWatch'>('watching')
    const state = reactive({
      ...
      totalPages: computed(() => {
        const list = state[currentTab.value as keyof typeof state] as any[]
    ...
    const getBangumiList = async () => {
      ...
      if (!id) {
        console.error('请先设置 Bangumi 用户 ID (vmid)')
        return
      }
      do {
        const req = await axios.get(
          `https://corsproxy.io/?${encodeURIComponent(`https://api.bgm.tv/v0/users/${id}/collections?subject_type=2&limit=${LIMIT}&offset=${offset}`)}`
        )
        ...
        for (const i of bangumisData) {
          ...
          const mergedData = {
            ...
            collection_info: { wish: 0, doing: 0, collect: 0, on_hold: 0 },
            subject_id,
            subject_type
          }
          if (i.type === 1) {
        ...
        // 并行获取收藏统计
        const allItems = [...state.wantWatch, ...state.watching, ...state.watched]
        const chunks = [] as { subject_id: number }[][]
        for (let k = 0; k < allItems.length; k += 5) chunks.push(allItems.slice(k, k + 5))
        for (const chunk of chunks) {
          await Promise.all(chunk.map(async item => {
            try {
              item.collection_info = await ViewerData(item.subject_id)
            } catch { /* use default */ }
          }))
        }
        offset += LIMIT
    ...
    const ViewerData = async (subject_id: number) => {
      try {
        const res = await axios.get(`https://corsproxy.io/?${encodeURIComponent(`https://api.bgm.tv/subject/${subject_id}`)}`)
